...
| *Method 1* + one `supplemental_files` directory

| Theme version in `package.json` and many custom partials on Default UI
| *Method 2* — npm supplemental overlay

| Fork or maintain your own copy of theme assets
| *Method 4* — vendor `supplemental-ui`
|===

IMPORTANT: Antora accepts *one* supplemental directory *or* a virtual-file array—not a YAML list of multiple directories.
The https://www.npmjs.com/package/antora-dark-theme[npm package] publishes the same `supplemental-ui` tree for *Method 2* and for theme development; most documentation sites should prefer the release bundle (*Method 1*).
...
=== Method 2: Use as npm dependency (advanced)
