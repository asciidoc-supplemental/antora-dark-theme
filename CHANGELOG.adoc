...
* *Documentation:* Recommend the GitHub release `ui-bundle.zip` as the default install path; document npm `supplemental-ui` as an advanced overlay on Antora Default UI. Clarify that Antora does not support multiple supplemental directories and that copying from `node_modules` on every build is unnecessary when using the bundle.
